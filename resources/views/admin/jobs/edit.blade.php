...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-slate-800">Edit Job: {{ $job->title }}</h2>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.jobs.update', $job) }}" method="POST" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">{{ old('description', $job->description) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="requirements" id="requirements" rows="4"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">{{ old('requirements', $job->requirements) }}</textarea>
