...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-slate-800">Add News Article</h2>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.news.store') }}" method="POST" class="p-6 md:p-8 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="News" {{ old('category') == 'News' ? 'selected' : '' }}>Company News</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="content" id="content" rows="8" required
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">{{ old('content') }}</textarea>
