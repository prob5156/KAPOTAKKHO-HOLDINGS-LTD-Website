...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-slate-800">Edit FAQ</h2>
    </div>

    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.faqs.update', $faq) }}" method="POST" class="p-6 md:p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="answer" id="answer" rows="5" required
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">{{ old('answer', $faq->answer) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $faq->display_order) }}" min="0" required
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">
