            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($contacts as $contact)
                        <tr class="hover:bg-slate-50/50 transition-colors {{ $contact->status === 'Unread' ? 'bg-amber-50/30' : '' }}">
                            <td class="px-6 py-4">
                                <div class="font-medium text-slate-900">{{ $contact->name }}</div>
                                <div class="text-xs text-slate-500">{{ $contact->email }}</div>
                            </td>
                            <td class="px-6 py-4 font-medium text-slate-800">{{ $contact->subject }}</td>
                            <td class="px-6 py-4 text-xs text-slate-500">{{ $contact->created_at->format('M d, Y h:i A') }}</td>
                            <td class="px-6 py-4">
                                @php
                                    $statusClasses = [
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.contacts.show', $contact) }}" class="text-indigo-600 hover:text-indigo-900 mx-2 font-medium">View</a>
                                <form action="{{ route('admin.contacts.destroy', $contact) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this message?');">
                                    @csrf
                                    @method('DELETE')
