...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-4 w-full sm:w-auto">
            <form action="{{ route('admin.projects.index') }}" method="GET" class="flex-1 sm:w-64">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search projects..." class="w-full px-4 py-2 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
            </form>
            <a href="{{ route('admin.projects.create') }}" class="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2.5 rounded-xl font-medium text-sm transition-colors shadow-sm flex items-center gap-2 whitespace-nowrap">
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($projects as $project)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">{{ $project->client ?? 'Internal' }}</td>
                            <td class="px-6 py-4">{{ $project->start_date ? $project->start_date->format('M d, Y') : 'N/A' }}</td>
                            <td class="px-6 py-4">{{ $project->end_date ? $project->end_date->format('M d, Y') : 'N/A' }}</td>
                            <td class="px-6 py-4">
                                @php
                                    $statusClasses = [
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.projects.edit', $project) }}" class="text-indigo-600 hover:text-indigo-900 mx-2 font-medium">Edit</a>
